...
contract('LoanFactory', (accounts) => {
  ...
  it('newLoan', async() => {
    ...
    loanAddress = await instance.loans(1);
    ...
    let addLenders = await instance.addLenders(loanAddress, lenders1, lenders2);
    let interest1 = [];
    let interest2 = [];
    ...
    let addInterest = await instance.addInterest(loanAddress, interest1, interest2);
    let start = await instance.start(loanAddress);
    status = await loan.status.call();
  ...
  it('addTransferOutcomeRecords', async() => {
    ...
    let addTransferOutcomeRecords = await instance.addTransferOutcomeRecords(loanAddress, records);
  });
  it('payInterest', async() => {
    let payInterest = await instance.payInterest(loanAddress, 0);
  ...
  it('interestPaid', async() => {
    let interestPaid = await instance.interestPaid(loanAddress, 0);
    let status = await loan.status.call();
    assert.strictEqual(status.toString(), '1');
  });
  it('mature', async() => {
    let mature = await instance.mature(loanAddress);
    let status = await loan.status.call();
