let Factory = artifacts.require('NoLibrary');
let Child = artifacts.require('NoLibraryChild');
let factory;
let child;
let tx;

contract('NoLibrary', (accounts) => {
  ...
  it('store10Uint', async() => {
    ...
    let varUint01 = await child.varUint01.call();
    assert.strictEqual(varUint01.toString(), uint10[0]);
    for (let i = 0; i < 10; i++) {
      uint10.push(web3.toBigNumber(i * 30 + 1).times('1e18').toString());
    }
    tx = await child.store10Uint(uint10);
    varUint01 = await child.varUint01.call();
    assert.strictEqual(varUint01.toString(), uint10[0]);
    for (let i = 0; i < 10; i++) {
  ...
  it('store10Bytes', async() => {
    ...
    let varBytes01 = await child.varBytes01.call();
    assert.strictEqual(varBytes01.toString(), bytes10[0]);
    for (let i = 0; i < 10; i++) {
      bytes10.push(web3.sha3(web3.toBigNumber(i * 30 + 1).times('1e18').toString()));
    }
    tx = await child.store10Bytes(bytes10);
    varBytes01 = await child.varBytes01.call();
    assert.strictEqual(varBytes01.toString(), bytes10[0]);
    for (let i = 0; i < 10; i++) {
  ...
  it('change10Enum', async() => {
    ...
    let status = await child.status.call();
    assert.strictEqual(status.toString(), '9');
    tx = await child.change10Enum();
    status = await child.status.call();
    assert.strictEqual(status.toString(), '9');
    tx = await child.change10Enum();
    status = await child.status.call();
    assert.strictEqual(status.toString(), '9');
