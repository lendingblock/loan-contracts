...
contract('lenders0interests10', () => {
  it('should create loan with 0 lenders and 10 interests', async () => {
    const timestamp = new Date().getTime();
    loanFactory = await LoanFactory.deployed();
    loan = loanGenerator({ id: 'id2', lendersCount: 0, interestsCount: 10 });
    ...
    assert.strictEqual(JSON.parse(tx.logs[0].args.loanMeta).interests.length, 10);
