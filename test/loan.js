...
contract('Loan', () => {
  it('should emit `TransferExpected` event with correct parameters when `expectTransfer()` called', async () => {
    ...
    const tx = await Loan.at(address).expectTransfer(loan.meta.borrowerUserId, loan.meta.holdingUserId, loan.meta.collateralAmount, loan.meta.collateralCurrency, 'initiation', timestamp);
